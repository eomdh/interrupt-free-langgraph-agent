...
def make_tag(llm: LLM) -> NodeFn:
    ...
    **알려진 한계 — 품질 축이 사람 라벨보다 엄하다.** `기여도` 55%, `문제해결`
    64%로 전부 헛경보 방향이다(`evals/README.md`). 미달 축 목록은 그대로 재작성
    힌트가 되므로(`_shortfalls` → `_REVISION`), 헛경보는 멀쩡한 부분까지 고치게
    만들어 ADR 0003의 "통과한 축은 유지한다"를 깎고 `MAX_REVISE` 예산을 태운다.
