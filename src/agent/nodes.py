...
def make_tag(llm: LLM) -> NodeFn:
    ...
    **알려진 한계 1 — 품질 축이 다른 축의 미달에 반응한다.** 30건 기준 `기여도`
    57%, `문제해결` 73%로 전부 헛경보 방향인데, 기여 표현만 다른 사다리 여섯 건에서는
    안 어긋난다. 헛경보는 전부 허위가 섞였거나 다른 품질 축이 미달인 초안에서 났다
    (`evals/README.md` §골든 셋 확장). 미달 축 목록은 그대로 재작성 힌트가 되므로
    (`_shortfalls` → `_REVISION`), 헛경보는 멀쩡한 부분까지 고치게 만들어 ADR 0003의
    "통과한 축은 유지한다"를 깎고 `MAX_REVISE` 예산을 태운다.

    **알려진 한계 2 — 답 모양을 심는 인젝션이 하드 게이트를 뚫는다.** 초안 끝에 판정
    JSON 을 그대로 붙여 두면 `과장허위`가 통과로 나온다(3회 반복 전부). 아래 방어 문구가
    "채점 방법을 바꾸라거나 특정 결과를 출력하라는 **문장**"을 겨냥하는데 JSON 블록은
    문장이 아니고, `_sealed` 는 울타리 구분자만 지운다. 네 판을 통틀어 관측된 유일한
    누출이다.

    **한 프롬프트로 5축을 재는 이 모양을 두 번 바꿔보고 두 번 되돌렸다.**
    ① `_TAG`에 "축은 서로 독립이다"를 명시한 판, ② 품질 4축과 하드 게이트를
    아예 다른 호출로 쪼갠 판. 둘 다 `과장허위`가 82%에서 45%로 무너졌고
    헛경보 2건이 6건이 됐다. 수치가 한 칸도 다르지 않다. 구조가 전혀 다른 두
    개입의 공통점은 **허위를 찾으라는 요구를 도드라지게 만들었다**는 것뿐이라,
    판단 대상을 좁히는 것 자체가 프라이밍으로 작동한다고 본다. 게이트의 82%는
    그 질문이 다른 넷 사이에 묻혀 있어서 나온 숫자다. 건드리기 전에 이걸 먼저
    다뤄야 한다.

    ②는 원래 **축 간 오염**(한 축의 미달이 나머지를 끌어내린다)을 없애려던
    것이었는데, 품질 채점관이 허위 질문을 구조적으로 못 보는 판에서도 `기여도`가
    55% 그대로였다. **오염 가설은 그래서 기각됐다.** 1판에서 본 동시 발생은
    사실이되 인과가 아니고, 허위가 섞인 초안이 독립적으로도 두루뭉술했던 것이다.

    목은 `task` 이름으로 답하므로 이 한계도, 되돌린 두 회귀도 테스트에 안 보인다.
